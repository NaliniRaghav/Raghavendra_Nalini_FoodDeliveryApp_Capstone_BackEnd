...
router.get('/cuisines', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: `Error retrieving cuisines: ${error.message}` });
  }
});
...
router.get('/', async (req, res) => {
  try {
    ...
    const [restaurants, totalRestaurants] = await Promise.all([
      Restaurant.find(filters).sort(sortOptions).skip((pageNum - 1) * limitNum).limit(limitNum).populate('menuItems'),
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: `Error deleting restaurant: ${error.message}` });
  }
});
